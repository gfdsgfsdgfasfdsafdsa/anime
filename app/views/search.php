<div class="body">
    <div class="anime-list-x-category">
        <div class="breadcrumb">
            ...
            <div class="breadcrumb-item active">Results for : <?= $_GET['keyword'] ?></div>
        </div>
        <?php if(!count($animes)): ?>
        <ul class="ml-4" style="font-size: 40px">No Results found for <?= $_GET['keyword'] ?></ul>
        <?php endif; ?>
        <ul>
        ...
        <div class="mt-n5 mb-5">
            <div class="d-flex justify-content-center align-items-center" style="margin-top: 100px;">
                <a href="<?php if($page != 1) echo 'search?keyword='.$_GET['keyword'].'&page='.($page-1) ?>" class="btn btn-primary px-2 mr-3 <?php if($page == 1) echo 'disabled' ?>"><i class="mr-2 fas fa-chevron-left"></i>Previous</a>
                <span class="mr-2">page</span>
                <form action="<?= ROOTURL ?>search" method="get" class="d-flex">
                ...
                <a href="<?php if($page != $totalPage) echo 'search?keyword='.$_GET['keyword'].'&page='.($page+1) ?>" class="btn btn-primary px-4 ml-3 <?php if($page == $totalPage || !count($animes)) echo 'disabled' ?>">Next<i class="ml-2 fas fa-chevron-right"></i></a>
            </div>
        </div>
    </div>